...
use crate::{token_kind_list, node_token_matches};
use crate::lexer::{Lexer, Token, TokenKind, DECLARATION_NAMES, TOKEN_KIND_CONSTRUCT_DELIMITERS};
...
impl<'a> Parser<'a> {
    pub fn new(lexer: Lexer<'a>) -> Self {
        ...
        parser.parse_loop(|parser, mut node| {
            ...
            node = parser.parse_tween(node).handle_construct_with_err(
                &mut parser.ast, &mut parser.ast_errors, &parser.lexer.rope, Some("the global scope")
            )?;
    ...
    fn parse_assignment(&mut self, node: Node<'a>) -> Parsed<'a> {
        let middle_node = match self.advance_until(token_kind_list![ Equals ], &TOKEN_KIND_CONSTRUCT_DELIMITERS) {
            Some(Ok(node)) => node,
    ...
    pub(crate) fn parse_loop<F: Fn(&mut Self, Node<'a>) -> Option<Node<'a>>>(&mut self, routine: F) -> Option<Node<'a>> {
        ...
        loop {
            let Some(next_node) = routine(self, node) else { break };
            node = next_node;
            ...
            if self.did_advance {
                ...
                let Some(next_node) = self.advance_without_flags()
                    .update_last_token_end(self) else { break };
                node = next_node;
            }
        }
